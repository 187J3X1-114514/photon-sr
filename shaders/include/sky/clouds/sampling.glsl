...
vec4 read_clouds_and_aurora(vec2 uv, out float apparent_distance) {
	...
	// colortex11/12 are at SR_SCALED resolution, get their actual size
	vec2 clouds_res = vec2(textureSize(colortex12, 0));
	
	// Soften clouds for new pixels
	float pixel_age = texelFetch(colortex12, ivec2(uv * clouds_res), 0).y;
	float ld = 2.0 * dampen(max0(1.0 - 0.1 * pixel_age));

	apparent_distance = min_of(textureGather(colortex12, uv, 0));
	vec4 result = textureLod(colortex11, uv, ld);

	if (LIGHTNING_FLASH_UNIFORM > 0.01) {
		float ambient_scattering = texture(colortex12, uv).z;
